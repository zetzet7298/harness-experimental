...
- `/var/www/vhcnd` is isolated from `/var/www/vltkunity` for the moved asset roots; E_M1 accepted.
- E_M2 spike report exists at `history/migrate-vltkpc-to-vhcnd/spike-name-hash.md` with verdict `no-go (re-plan)`.
- First US-011 dry-run attempted `vltk-audit-equipment-stat-coverage.py` with `--vltkpc-root /var/www/vhcnd/sources` and failed because catalog `source.path` values still point at `Client/Settings/item/*.txt`.
...
1. Catalog source metadata is readable through `/var/www/vhcnd/sources` without referring to `/var/www/vltkpc` or `/var/www/vltkunity`.
...
- `history/migrate-vltkpc-to-vhcnd/spike-scripts/e2r-source-metadata-evidence.md`
- `history/migrate-vltkpc-to-vhcnd/spike-scripts/e2r-audit-summary.json`
- `history/migrate-vltkpc-to-vhcnd/spike-scripts/outputs/*.audit.json`
...
python3 scripts/vltk-audit-equipment-stat-coverage.py   --catalog src/data/equipmentCatalog.json   --source src/domain/equipment.ts   --out /var/www/vltk-h5-survivors/harness-experimental/history/migrate-vltkpc-to-vhcnd/spike-scripts/outputs/e2r-equipment-stat-coverage.audit.json   --vltkpc-root /var/www/vhcnd/sources
```
...
- Verify every catalog `source.path` resolves below `/var/www/vhcnd/sources` and no catalog source path starts with `/var/www/vltkpc` or `/var/www/vltkunity`.
- Run a row-identity probe for the 12 source files and write the findings to `e2r-source-metadata-evidence.md`.
- Attempt remaining ordered US-011 audits from the existing US-011 `execplan.md`, redirecting all `--out` paths to `history/migrate-vltkpc-to-vhcnd/spike-scripts/outputs/`.

## Out of scope

- No harness-wide `vltkpc` → `vhcnd` textual sweep yet.
- No `.codex/skills/` rename yet.
- No GitNexus repo/group rename yet.
- No Playwright snapshot regeneration yet.
- No broad asset folder rename under `public/assets/*/vltkpc` yet.
