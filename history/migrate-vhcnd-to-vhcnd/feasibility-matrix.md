...
| --- | --- | --- | --- | --- |
| A1 | vltkunity and vhcnd are on the same filesystem so `mv` is an atomic rename. | `stat -c '%m' /var/www/vhcnd /var/www/vhcnd` | Both report `/`. | Pass. Move is rename. |
| A2 | vhcnd has 5 and only 5 callers of vltkunity paths. | `rg -l '/var/www/vhcnd' /var/www/vhcnd` | Returns exactly the 5 known: `web/manifest.json`, `web/manifest_data_cdn_lite.json`, `web/manifest_downloads_like.json`, `web/manifest_required_config_missing_only.{json,csv}`. | Pass. No hidden callers. |
| A3 | vltkunity does not consume vhcnd. | `rg -l '/var/www/vhcnd' /var/www/vhcnd` (excluding datasets/archives) | Zero matches. | Pass. Direction is one-way. |
| A4 | `tools/scan_required_spr.py` runs end-to-end and emits a JSON report. | `python3 tools/scan_required_spr.py --config-dir <vhcnd-real> --spr-root /var/www/vhcnd/item_spr` | Runs to completion. `total=335, missing=10, ok=325`. | Pass. Pre-move baseline = 10 missing. |
...
2. The E_M2 spike: probe script under `history/migrate-vhcnd-to-vhcnd/spike-scripts/`, audit dry-run output under the same folder, spike report at `history/migrate-vhcnd-to-vhcnd/spike-name-hash.md`.
...
- Proof surface: PASS (`history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-validation-evidence.md` and `e2r-validation-summary.json`).
