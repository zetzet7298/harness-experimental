...
that `rg /var/www/vhcnd /var/www/vhcnd` returns zero. Pre-flight grep
...
> divergence) … or `rg /var/www/vhcnd /var/www/vhcnd` is non-zero
...
| --- | --- | --- | --- | --- |
| `web/manifest.json` | top-level `root` | `/var/www/vhcnd/item_spr_img` | `/var/www/vhcnd/item_spr_img` | ✓ |
| `web/manifest_required_config_missing_only.json` | top-level `sprRoot` | `/var/www/vhcnd/item_spr` | `/var/www/vhcnd/item_spr` | ✓ |
| `web/manifest_required_config_missing_only.csv` | header row | `path,name,configFile,configLine,absPath` (no vltkunity) | `/var/www/vhcnd/item_spr` | **✗ divergence** |
| `web/manifest_data_cdn_lite.json` | top-level `root` | `/var/www/vhcnd/other-game/download-source/data_cdn_spr_img` | same | ✓ |
| `web/manifest_downloads_like.json` | top-level `root` | `/var/www/vhcnd/other-game/item_spr_like_img` | same | ✓ |
...
> | `web/manifest_required_config_missing_only.csv` | header containing path | `/var/www/vhcnd/item_spr` | `/var/www/vhcnd/item_spr` |
...
820 of the 4,686 data rows contain `/var/www/vhcnd/...` in the
...
"Spr/item/equip/bangle/obj-bangle32.spr","Minh Phượng Không Minh Tụ","/var/www/vhcnd/item_data/settings/item/004/goldequip.txt","2939","/var/www/vhcnd/item_spr/Spr/item/equip/bangle/obj-bangle32.spr"
"Spr/item/equip/bangle/obj-bangle33.spr","Độc Cô Kim Uyển","/var/www/vhcnd/item_data/settings/item/004/goldequip.txt","5663","/var/www/vhcnd/item_spr/Spr/item/equip/bangle/obj-bangle33.spr"

$ grep -c 'vltkunity' /var/www/vhcnd/web/manifest_required_config_missing_only.csv
820
```

A naïve `s|/var/www/vhcnd/item_spr|/var/www/vhcnd/item_spr|g`
...
> `rg /var/www/vhcnd /var/www/vhcnd` returns ZERO matches.
...
```
$ rg -l '/var/www/vhcnd' /var/www/vhcnd
...
| File | Reference type |
| --- | --- |
| `item_spr_img/manifest.json` | `root: "/var/www/vhcnd/item_spr_img"` (item_spr_img/ tree was moved; this manifest sits *inside* the moved tree as a duplicate of `web/manifest.json`) |
| `item_spr_img/manifest_data_cdn_lite.json` | `root: "/var/www/vhcnd/other-game/download-source/data_cdn_spr_img"` (duplicate of `web/manifest_data_cdn_lite.json`) |
| `item_spr_img/manifest_downloads.json` | `root: "/var/www/vhcnd/other-game/downloads_spr_img_struct"` — `downloads_spr_img_struct/` is NOT in D6 move scope (still in vltkunity) |
| `item_spr_img/manifest_downloads_like.json` | `root: "/var/www/vhcnd/other-game/item_spr_like_img"` (duplicate) |
| `item_spr_img/manifest_required_config.json` | `root: "/var/www/vhcnd/item_data/settings/item/004"` — `item_data/` is NOT in D6 move scope |
| `item_spr_img/manifest_required_config_scan_full.json` | `scanScope: "/var/www/vhcnd/item_data/settings/item"` AND `sprRoot: "/var/www/vhcnd/item_spr"` — first not moved, second moved |
| `other-game/item_spr_like_img/report.json` | `spr_root: "/var/www/vhcnd/other-game/item_spr_like"` (NOT moved) AND `out_root: "/var/www/vhcnd/other-game/item_spr_like_img"` (moved) |
...
A blanket `s|/var/www/vhcnd|/var/www/vhcnd|g` over these files would
...
2. **Acceptance #2 unachievable from the listed scope** — even with all
   ...
   still contain `/var/www/vhcnd` strings, some of which legitimately
...
A. **Tighten acceptance #2** to scope-bounded grep:
   ```
   rg /var/www/vhcnd /var/www/vhcnd/web /var/www/vhcnd/tools
   ```
...
$ node .codex/khuym_reservations.mjs reserve --agent "Patch" --bead "mig-piy" \
    ...
    --path history/migrate-vhcnd-to-vhcnd/spike-scripts/field-edit-evidence.md \
    --ttl 3600 --json
