...
Command:
```bash
find /var/www/vhcnd -type f \( -name '*.json' -o -name '*.csv' -o -name '*.html' -o -name '*.md' -o -name '*.txt' -o -name '*.py' -o -name '*.sh' -o -name '*.js' -o -name '*.mjs' -o -name '*.ts' -o -name '*.tsx' -o -name '*.yml' -o -name '*.yaml' -o -name '*.xml' \) -print0 | xargs -0 grep -Il '/var/www/vhcnd'
```
Output:
```text
```
...
try:
    ...
    if '/var/www/vhcnd' in text:
        failed = True
        print(f'FAIL csv contains /var/www/vhcnd: {csv_path}')
    else:
        print(f'PASS csv no /var/www/vhcnd: {csv_path}')
except Exception as exc:
    failed = True
...
PASS csv no /var/www/vhcnd: /var/www/vhcnd/web/manifest_required_config_missing_only.csv
...
```bash
set -o pipefail
if rg -I --fixed-strings --line-number '/var/www/vhcnd' /var/www/vhcnd; then exit 1; fi
python3 - <<'PY'
# JSON parse checks for known residual files.
# CSV header and no-/var/www/vhcnd check for web/manifest_required_config_missing_only.csv.
...
PASS text-aware scan no /var/www/vhcnd under /var/www/vhcnd
