...
- Chạy kiểm tra gate:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-gate-readiness-check.py`
- Xem kết quả:
...
- Dry-run (không sửa template):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-golditem-apply-repair-proposals.py`
- Apply thật (ghi vào template):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-golditem-apply-repair-proposals.py --apply`
- Apply + verify chain (validate + one-shot):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-golditem-apply-repair-proposals.py --apply --verify`
- Báo cáo thay đổi:
...
- Chạy mô phỏng áp repair proposals trên template preview:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-preview-repair-impact.py`
- Artifacts:
...
- Chạy forecast:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-gate-forecast.py`
- Output:
...
- Option B (provisional path execution):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-run-option-b.py`
...
- Precheck (không close):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-close-mig-ja5.py`
- Close thật (chỉ chạy khi gate ready):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-close-mig-ja5.py --confirm --reason "<reason>"`
- Close + ghi addendum vào verdict:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-close-mig-ja5.py --confirm --reason "<reason>" --append-verdict-note`
- Report:
...
- Precheck (không có approve sẽ không làm gì):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-finalize-provisional.py`
- Finalize thật (cần human explicit):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-finalize-provisional.py --human-approved --decided-by "<human>" --reason "<reason>"`
- Report:
...
- Chạy generator:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-generate-next-step.py`
- Output:
  - `e2r-next-step.md`
  - `e2r-next-step.json`
...
- Chạy wrapper:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-finalize-and-audit.py`
- Nếu đã có human approval:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-finalize-and-audit.py --human-approved --decided-by "<human>" --reason "..."`
- Output:
...
- Preview receipt (không ghi decision file):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-capture-human-decision.py --decided-by "<human>" --mode accept-provisional --note "..."`
- Apply vào `e2r-gate-decision.json`:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-capture-human-decision.py --decided-by "<human>" --mode accept-provisional --note "..." --apply`
- Artifacts:
...
- Preview orchestration:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-approve-and-finalize.py --decided-by "<human>" --mode accept-provisional --note "..."`
- Apply thật (decision + finalize chain):
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-approve-and-finalize.py --decided-by "<human>" --mode accept-provisional --note "..." --apply`
- Output:
...
- Chạy kiểm tra toàn vẹn gói handoff:
  - `python3 history/migrate-vhcnd-to-vhcnd/spike-scripts/e2r-artifact-integrity-check.py`
- Output:
