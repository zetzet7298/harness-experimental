# Discovery — migrate-vhcnd-to-vhcnd
...
| `/var/www/vhcnd` | gitnexus repo `vhcnd` (group `@vltk-porting` member `pc`) | Legacy PC source mirror. Source of US-001..US-011 evidence. |
...
| `/var/www/vhcnd` | not indexed | Umbrella that previously held named SPR/PNG that vhcnd manifests reference. Per D5/D6, the four paths move into vhcnd. |
...
- 38 files reference `vhcnd` (counted by `rg -i vhcnd -l`).
- Existing skill files referencing vhcnd/vltk-porting/etc:
  `.codex/skills/vltk-{item-research, skill-porting, map-porting, spr-porting}/`.
- Story epic folder at `docs/stories/epics/E03-vhcnd-asset-porting/`.
- TEST_MATRIX has many implemented rows under E03; rebrand must preserve
  evidence.

### Game source

- 430 files reference `vhcnd` (counted by `rg -i vhcnd -l`).
- Hardcoded out-of-scope paths under `/var/www/vhcnd/extracted_build_jxwin_soucgoc_hoiquanvolam/source_from_vmdk/SourceNew/swrod3/bin/Client/Settings/item/` in:
  ...
  - `scripts/check-no-runtime-vhcnd.sh`
- Internal identifiers: animation prefix `player-{run,idle}-mounted-vhcnd-h{H}-a{A}`,
  localStorage key `vhcnd-wardrobe-selection-v1`, `playerSheet` constant,
  asset folder `public/assets/character/vhcnd/`, `public/assets/skills/vhcnd/`,
  `public/assets/maps/vhcnd/`.
- Two files reference `vltkunity` instead of `vhcnd`:
...
- `web/manifest*.json` files reference `/var/www/vhcnd/...` as their `root`/`sprRoot`.
...
- `/var/www/vhcnd/item_spr/` — 28 MB, 1,627 named SPR (e.g., `spr/item/equip/horse/horse003.spr`).
- `/var/www/vhcnd/item_spr_img/` — 82 MB, 13,557 named PNG.
- `/var/www/vhcnd/other-game/item_spr_like_img/` — 4.4 MB, 672 entries.
- `/var/www/vhcnd/other-game/download-source/data_cdn_spr_img/` — referenced in vhcnd manifest with 42,809 entries; the directory does NOT exist on disk (metadata-only manifest).
- Reverse direction: `rg /var/www/vhcnd /var/www/vhcnd` returns zero references; vltkunity does not consume anything from vhcnd. The migration direction is one-way.
