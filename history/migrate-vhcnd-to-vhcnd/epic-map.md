# Epic Map — migrate-vhcnd-to-vhcnd
...
- **Work:**
  - Move `/var/www/vhcnd/item_spr/` → `/var/www/vhcnd/item_spr/`.
  - Move `/var/www/vhcnd/item_spr_img/` → `/var/www/vhcnd/item_spr_img/`.
  - Move `/var/www/vhcnd/other-game/item_spr_like_img/` → `/var/www/vhcnd/other-game/item_spr_like_img/`.
  - Move (or recreate, since it is metadata-only) `/var/www/vhcnd/other-game/download-source/data_cdn_spr_img/` → `/var/www/vhcnd/other-game/download-source/data_cdn_spr_img/`.
...
- **Exit:**
  - `rg /var/www/vhcnd /var/www/vhcnd` returns zero.
...
- **Exit:**
  - Spike report at `history/migrate-vhcnd-to-vhcnd/spike-name-hash.md` with: hash mapping verdict, audit gap summary, go/no-go for E_M3..E_M7, and (if no-go) the smallest remediation plan.
...
- **Capability:** make `src/data/equipmentCatalog.json` and the US-011 audit scripts use readable, non-`vhcnd`, non-`vltkunity` source provenance under `/var/www/vhcnd/sources` before any broad rebrand sweep.
- **Entry:** E_M1 accepted; E_M2 spike report says `no-go (re-plan)` specifically because all 1,231 catalog source-line checks are `source-file-unreadable` when `--vhcnd-root` is redirected to `/var/www/vhcnd/sources`.
- **Work:**
  ...
  - Rerun the first stat audit and then the remaining ordered US-011 audits with output redirected under `history/migrate-vhcnd-to-vhcnd/spike-scripts/outputs/`.
- **Exit:**
  ...
  - No runtime code reads from `/var/www/vhcnd` or `/var/www/vhcnd`; any source files needed for H5 provenance are copied/translated into game-source artifacts or read from `/var/www/vhcnd` only during tooling.
...
- **Capability:** make the agent harness say `vhcnd` everywhere it now says `vhcnd`, including epic folder structure, AGENTS.md routing, decision records, and skill SKILL.md files.
- **Entry:** E_M2R exit accepted.
- **Work:**
  - Rewrite the 38 harness files from the scout (AGENTS.md, README.md, docs/HARNESS.md, docs/product/*, docs/decisions/0004, docs/stories/backlog.md, docs/stories/epics/E03-vhcnd-asset-porting/*, docs/validation/*.md, docs/TEST_MATRIX.md).
  - Rename `docs/stories/epics/E03-vhcnd-asset-porting/` to `docs/stories/epics/E03-vhcnd-asset-porting/` and rewrite epic README.
  ...
  - Add a new story packet under the renamed epic for this migration (story slug `migrate-vhcnd-to-vhcnd`) once beads are scheduled.
  - Update `docs/HARNESS_BACKLOG.md` if any harness improvements surface.
- **Exit:** `git ls-files | xargs grep -l vhcnd` in harness-experimental returns zero (excluding history snapshots that intentionally retain old names).
...
- **Capability:** make every game-source script and config that previously read `/var/www/vhcnd/...` read from the corresponding vhcnd location.
...
- **Work:**
  ...
  - Rewrite each script that hardcodes `/var/www/vhcnd/...` to import the new module.
  - Rename and update `scripts/check-no-runtime-vhcnd.sh` → `scripts/check-no-runtime-vhcnd.sh`. Update its needle, its message, and every doc/CI/validation file that references the old name.
- **Exit:** `rg /var/www/vhcnd /var/www/vltk-h5-survivors/game-source` returns zero; `python3 -m py_compile scripts/*.py` passes.
- **Risk absorbed:** R3, R5.
- **Gating:** Gate 3.
...
- **Capability:** rename internal paths, asset keys, animation prefixes, and the localStorage key so the runtime no longer says `vhcnd` anywhere.
- **Entry:** E_M4 done.
- **Work:**
  - Rename directories: `public/assets/character/vhcnd/` → `vhcnd/`, `public/assets/skills/vhcnd/` → `vhcnd/`, `public/assets/maps/vhcnd/` → `vhcnd/`. Update every `fetch('/assets/...')` call in `src/game/scenes/*.ts` and every script that writes there.
  - Rename animation prefix templates `player-run-mounted-vhcnd-h{H}-a{A}` → `player-run-mounted-vhcnd-h{H}-a{A}`. Update the prefix-generating code in `GameScene.ts` and any animation registry.
  - Migrate localStorage key `vhcnd-wardrobe-selection-v1` → `vhcnd-wardrobe-selection-v1`. Implement a one-shot shim that reads the old key on boot, writes the new one, and deletes the old. Default = include the shim. (R4.)
  - Rename asset keys (`vhcnd-equipped-tu-la-...`), file slug fields in port packets and audits where `vhcnd` appears in slug strings, and update generators so freshly produced packets use the new slug.
  - Update `playerSheet` constant in `src/game/constants.ts`.
- **Exit:** `rg vhcnd /var/www/vltk-h5-survivors/game-source` returns zero; `npm run typecheck` and `npm run build` pass.
- **Risk absorbed:** R4.
- **Gating:** Gate 3.
...
- **Capability:** make tooling discovery match the new naming and prove `/var/www/vhcnd/` is no longer required by anything in the migrated surfaces.
- **Entry:** E_M6 done.
- **Work:**
  - Re-register or rename the gitnexus repo: per D3 default, `vhcnd` → `vhcnd`. Game-source repo decision deferred pending OQ-3 (rename `vltk-h5-survivors` to `vhcnd-h5-survivors` is implied but not yet locked; planning records this as an open question).
  ...
  - Final isolation probe: `rg /var/www/vhcnd` and `rg /var/www/vhcnd` across `/var/www/vhcnd`, `/var/www/vltk-h5-survivors/harness-experimental`, `/var/www/vltk-h5-survivors/game-source`. Both must return zero. Record the proof in the validation report.
