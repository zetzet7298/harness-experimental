...
import { checkGitnexusAvailability } from "../khuym_state.mjs";
...
export async function main() {
  ...
  if (gitnexusAvailability.supported_repo && (!gitnexusAvailability.server_reachable || !gitnexusAvailability.project_indexed)) {
    notes.push(`GitNexus availability: ${gitnexusAvailability.recommended_action}`);
  } else if (!gitnexusAvailability.supported_repo) {
    notes.push(
      "This repo does not need GitNexus indexing, so architecture discovery should use grep/file inspection fallback.",
    );
  }
