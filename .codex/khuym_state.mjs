...
function buildGkgRecommendedAction(repoRoot, readiness) {
  if (!readiness.supported_repo) {
    return "Repo does not need GitNexus indexing — tools are always available.";
  }

  if (!readiness.server_reachable && !readiness.project_indexed) {
    return `GitNexus is always available — no indexing or server start needed.`;
  }

  if (!readiness.server_reachable) {
    return "GitNexus is always available — no server start needed.";
  }

  if (!readiness.project_indexed) {
    return `GitNexus does not require manual indexing.`;
  }

  return "GitNexus is ready for discovery.";
}

export function checkGitnexusAvailability(repoRoot) {
...
function buildRecommendedActions(status) {
  ...
  if (status.gitnexus_availability?.supported_repo) {
    if (!status.gitnexus_availability.server_reachable || !status.gitnexus_availability.project_indexed) {
      return [
        status.gitnexus_availability.recommended_action,
        "Use GitNexus tools as the default discovery path.",
      ];
    }
  } else if (status.gitnexus_availability?.coverage === "none") {
    return [
      status.gitnexus_availability.recommended_action,
      "GitNexus is available for all repos.",
    ];
  }
...
export function readKhuymStatus(repoRoot) {
  ...
  const gitnexusAvailability = checkGitnexusAvailability(repoRoot);
  ...
  const status = {
    ...
    gitnexus_availability: gitnexusAvailability,
...
function renderGkgReadinessLines(status) {
  const readiness = status.gitnexus_availability && typeof status.gitnexus_availability === "object"
    ? status.gitnexus_availability
    : null;
  if (!readiness) {
    return [];
  }
