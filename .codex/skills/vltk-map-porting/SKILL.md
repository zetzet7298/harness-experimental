...
Never make the H5 runtime read `/var/www/vltkpc` directly. Copy or generate runtime assets under `/var/www/vltk-h5-survivors/game-source`.

## Required Context

Work from `/var/www/vltk-h5-survivors/game-source` for game code and assets. Use `/var/www/vltkpc` only as PC evidence. Use explicit GitNexus repos when querying:
...
3. Decode/copy source assets into `game-source`; do not wire assets from `/var/www/vltkpc` at runtime.
...
6. Generate tiled runtime backgrounds and metadata under `public/assets/maps/vltkpc/<slug>/` and `data/vltk-normalized/maps/`.
...
- Resolve enemy SPRs through NpcRes table evidence and copy SPRs into `public/assets/enemies/vltkpc/` before runtime use.
...
Capture a screenshot or a crop from `public/assets/maps/vltkpc/<slug>/background.jpg` when visual scale/aspect changed.
