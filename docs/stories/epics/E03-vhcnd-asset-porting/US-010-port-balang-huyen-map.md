...
Port VHCND map `Ba Lăng huyện` into the H5 runtime with source-backed map identity, copied local map/minimap assets, generated map metadata, and only mobile combat enemy templates. NPCs are not ported and the minimap must not render NPC names.
...
- Active VHCND map evidence identifies `Ba Lăng huyện` as `mapId=53`, path `两湖区\巴陵县`, `MapType=Country`, and `MapPos=351,391`.
- H5 runtime assets are copied or generated under `game-source`; runtime does not read from `/var/www/vhcnd`.
- Map metadata records `.wor` rect `82,89,114,108`, PC region size `512x1024`, and H5 world size `16896x20480`.
- Minimap uses the VHCND `24.jpg` map image and renders player/enemy dots only, with no NPC labels or NPC names.
...
- Runtime background must be rendered from VHCND `Region_C.dat` / `Ground.dat`; using `24.jpg` minimap as the world background is invalid because it creates a broken mosaic at gameplay zoom.
...
- Updated `docs/product/vhcnd-porting.md` so map ports have explicit gates beside item/skill ports.
...
- Copied map assets: `game-source/public/assets/maps/vhcnd/balang-huyen/`.
- Copied/generated enemy assets: `game-source/public/assets/enemies/vhcnd/`.
