...
Validated `US-010` map port for VHCND `Ba Lăng huyện`: map identity, minimap image, copied runtime assets, mobile enemy templates, H5 runtime loading, and browser visual smoke.
...
- `game-source/public/assets/maps/vhcnd/balang-huyen/minimap.jpg` and `background.jpg`.
- `game-source/public/assets/enemies/vhcnd/ani049-walk.png`, `ani061-walk.png`, and `ani063-walk.png` plus metadata JSON.
...
- Browser dev output still reports pre-existing missing-frame warnings for `vhcnd-equipped-tu-la-giang-sa-staff-phien-vu-idle/run`; this belongs to the equipped-character sprite sheet metadata path, not the Ba Lăng map port.
- The H5 survivor adapter ports enemy types and map/minimap visuals, not exact VHCND NPC dialog behavior or NPC labels. NPCs are intentionally excluded by request.
