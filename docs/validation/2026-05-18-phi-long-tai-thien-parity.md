...
- Package order: `/var/www/vhcnd/extracted_build_jxwin_soucgoc_hoiquanvolam/source_from_vmdk/SourceNew/swrod3/bin/Client/package.ini:3-31`, where `slistcache.pak` is the first active source for the extracted rows.
...
- `Param2PCoordinate`: `/var/www/vhcnd/extracted_build_jxwin_soucgoc_hoiquanvolam/source_from_vmdk/SourceNew/swrod3/SwordOnline/Sources/Core/Src/KSkills.cpp:93-128`.
- `CastWall`: `/var/www/vhcnd/extracted_build_jxwin_soucgoc_hoiquanvolam/source_from_vmdk/SourceNew/swrod3/SwordOnline/Sources/Core/Src/KSkills.cpp:604-665` and `:1398-1514`.
- `MoveKind=5` Follow: `/var/www/vhcnd/extracted_build_jxwin_soucgoc_hoiquanvolam/source_from_vmdk/SourceNew/swrod3/SwordOnline/Sources/Core/Src/Skill/SkillDef.h:104-115`; movement update in `/var/www/vhcnd/extracted_build_jxwin_soucgoc_hoiquanvolam/source_from_vmdk/SourceNew/swrod3/SwordOnline/Sources/Core/Src/KMissle.cpp:745-767`.
...
PC `CastWall` centers the wall on a target/reference coordinate. In VHCND this coordinate comes from the player cast target or click point. In the H5 survivor loop, target selection is automatic and can be far from the character, which made THVC/FLTT appear to originate at the enemy. For these forward-launch Cái Bang Wall/Follow skills, H5 now uses:
...
- FLTT runtime main sheet/meta: `/var/www/vltk-h5-survivors/game-source/public/assets/skills/vhcnd/phi-long-tai-thien-main.png` and `.json`.
...
- `projectileTextureKey=vhcnd-bong-da-ac-cau-main`.
...
- `projectileTextureKey=vhcnd-thien-ha-vo-cau-main`.
...
Human runtime review identified the missing enemy-hit visual: VHCND creates a collision special effect, not only a hit sound. Evidence used:
